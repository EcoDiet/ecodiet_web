...
class _CreateAccountPageState extends State<CreateAccountPage> {
  ...
  static const Map<String, int> _dietToRegimeId = {
    'omnivore':    1, // carnivore
    'vegetarian':  4,
    'vegan':       3,
    'pescatarian': 2, // pescetarian
    'flexitarian': 4, // nearest equivalent: vegetarian
  };

  static const Map<String, int> _allergyToId = {
    'gluten':   5,
    'lactose':  6,
    'nuts':     4,  // fruits_a_coque
    'seafood':  3,  // crustaces
    'eggs':     9,  // oeuf
    'soy':      12, // soja
    'peanuts':  1,  // arachide
  ...
  Future<void> _finishOnboarding() async {
    ...
    if (!result.isSuccess) {
      ...
      if (mounted) _showError(result.error ?? 'Erreur lors de la création du compte');
      return;
    }

    // Step 2 — save dietary regime
    if (_dietType != null) {
      final regimeId = _dietToRegimeId[_dietType];
      if (regimeId != null) {
        await _api.setUserRegime(regimeId);
      }
    }

    // Step 3 — save allergies
    for (final allergy in _allergies) {
      final allergeneId = _allergyToId[allergy];
      if (allergeneId != null) {
        await _api.addUserAllergene(allergeneId);
      }
    }

    // Step 4 — save extra profile fields in auth metadata
    await _api.updateUserMetadata({
      if (_gender != null)           'gender':        _gender,
      if (_birthDate != null)        'birth_date':    _birthDate!.toIso8601String(),
      if (_heightController.text.isNotEmpty) 'height_cm': _heightController.text,
      if (_weightController.text.isNotEmpty) 'weight_kg': _weightController.text,
      if (_targetWeightController.text.isNotEmpty) 'target_weight_kg': _targetWeightController.text,
      if (_mainGoal != null)         'goal':          _mainGoal,
      if (_dietType != null)         'diet_type':     _dietType,
    });
